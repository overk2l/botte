...
client.on("interactionCreate", async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    if (ctx === "type") {
      ...
      if (action === "dropdown") types = ["dropdown"];
      if (action === "button") types = ["button"];
      if (action === "both") types = ["dropdown", "button"];

      db.saveSelectionType(menuId, types);

      // Start with dropdown role selection if needed
      if (types.includes("dropdown")) {
        return promptRoleSelect(interaction, menuId, "dropdown");
      }
  ...
  if (interaction.isModalSubmit() && interaction.customId === "rr:modal:create") {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`type:dropdown::${menuId}`).setLabel("Use Dropdown").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`type:button::${menuId}`).setLabel("Use Buttons").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId(`type:both::${menuId}`).setLabel("Use Both").setStyle(ButtonStyle.Success)
    );

    return interaction.reply({ content: "✅ Menu created. Choose how users will select roles:", components: [row], flags: 64 });
  }
  ...
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith("rr:roles:")) {
    ...
    db.saveRoles(menuId, type, interaction.values);

    const menu = db.getMenu(menuId);
    const nextStep = type === "dropdown" && menu.selectionType.includes("button");
    ...
    return interaction.update({ content: "✅ Roles saved! Ready to publish?", components: [row] });
  }
  ...
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith("rr:use:")) {
    ...
    return interaction.reply({ content: "✅ Your dropdown roles have been updated!", flags: 64 });
  }
...
async function promptRoleSelect(interaction, menuId, type) {
  ...
  await interaction.reply({ content: `Select roles for **${type}**:`, components: [row], flags: 64 });
}
